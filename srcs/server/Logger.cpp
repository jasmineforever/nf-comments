...
const char* Logger::_logTyeString[] = { "[fatal]", "[warning]", "[trace]"};
...
void Logger::writeLog(LogType type, const char* szFormat, ...)
{
	...
	if (_isShowTimestamp)
	{
		...
		SYSTEMTIME start;
		GetLocalTime(&start);
		nowSize = SPRINTF(_writeBuf, _writeBufSize, "[%4d-%.2d-%.2d %.2d:%.2d:%.2d:%.4d]", start.wYear, start.wMonth, start.wDay, start.wHour, start.wMinute, start.wSecond, start.wMilliseconds);
#elif defined(linux)
        struct timeval   tv;
